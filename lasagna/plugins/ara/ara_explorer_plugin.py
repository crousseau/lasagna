...
class plugin(AraPluginBase, ara_explorer_UI.Ui_ara_explorer):
    ...
    def setup_tree(self):
        ...
        self.brainArea_treeView.selectionModel().selectionChanged[QtCore.QItemSelection, QtCore.QItemSelection].connect(
            self.highlightSelectedAreaFromList)
    ...
    def get_atlas_image_stack(self, plugin_name=None):
        ...
        if not ingredient:
            if plugin_name is not None:
                print("ARA_explorer_plugin.{} Failed to find image_stack named {}"
                      .format(plugin_name, atlas_layer_name))
            return None, None
        else:
